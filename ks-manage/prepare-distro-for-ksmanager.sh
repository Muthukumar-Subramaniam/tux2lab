...
fn_get_iso_url() {
    ...
    # RHEL requires manual download + subscription credentials
    if [[ "$distro" == "rhel" ]]; then
        local iso_file="${ISO_FILENAMES[${distro}:${version}]:-}"
        local iso_path="${ISO_DIR}/${iso_file}"
        local rhel_sub_file="/tux2lab-data/lab-config/rhel-subscription.conf"

        print_yellow "
Red Hat Enterprise Linux requires a subscription for ISO download
and PXE installation. A free Developer subscription is sufficient.

  Sign up:          https://developers.redhat.com/register
  Download ISO:     https://access.redhat.com/downloads/content/rhel
  Activation Keys:  https://console.redhat.com/settings/connector/activation-keys

You need:
  1. Boot ISO download URL (copy from the download page)
  2. SHA-256 checksum (shown on the download page, optional)
  3. Organization ID (shown at the top of the Activation Keys page)
  4. Activation Key (create one, e.g. 'tux2lab')
" >&2

        read -rp "Boot ISO download URL (or Enter if ISO placed at ${iso_path}): " iso_url
        if [[ -n "$iso_url" ]]; then
            local url_basename
            url_basename=$(basename "${iso_url%%\?*}")
            if [[ "$url_basename" != *boot* ]]; then
                print_error "Not a Boot ISO (filename must contain 'boot'): ${url_basename}"
                exit 1
            fi
            if [[ "$url_basename" != rhel-${version}* ]]; then
                print_error "ISO does not match RHEL ${version}: ${url_basename}"
                exit 1
            fi
        ...
        read -rp "SHA-256 checksum (or Enter to skip verification): " rhel_checksum
        if [[ -n "$rhel_checksum" ]]; then
            local checksum_file="${ISO_DIR}/${distro}-${version}-CHECKSUM"
            echo "${rhel_checksum}  ${iso_file}" > "$checksum_file"
        fi

        # Collect subscription credentials if not already saved
        if [[ ! -f "$rhel_sub_file" ]]; then
            echo >&2
            read -rp "Organization ID: " rhel_org_id
            if [[ -z "$rhel_org_id" ]]; then
                print_error "Organization ID is required for RHEL PXE installs."
                exit 1
            fi
            read -rp "Activation Key: " rhel_activation_key
            if [[ -z "$rhel_activation_key" ]]; then
                print_error "Activation Key is required for RHEL PXE installs."
                exit 1
            fi
            mkdir -p "$(dirname "$rhel_sub_file")"
            cat > "$rhel_sub_file" << EOF
# RHEL Subscription Manager credentials for PXE kickstart installs
# Generated by: tux2lab distro setup rhel
RHEL_ORG_ID=${rhel_org_id}
RHEL_ACTIVATION_KEY=${rhel_activation_key}
EOF
            chmod 644 "$rhel_sub_file"
            print_success "Subscription credentials saved." >&2
        else
            print_info "Subscription credentials already configured." >&2
        fi

        if [[ -n "$iso_url" ]]; then
            echo "$iso_url"
        elif [[ -f "$iso_path" ]]; then
            print_success "Found manually placed ISO: ${iso_path}" >&2
            echo ""
        fi
        return
    fi
...
fn_setup_distro() {
    ...
    # For RHEL: URL basename differs from saved filename, use iso_file directly
    ...
    if [[ -n "$checksum_url" ]]; then
        ...
        # No checksum URL — check if a local checksum file was provided (e.g., RHEL manual input)
        if [[ -f "$checksum_file" ]]; then
            ...
            print_warning "No checksum available for ${DISTRO_DISPLAY_NAMES[$distro]} ${version}. Skipping verification."
        fi
    fi
